...
const firstInputId = 'owner-sparepart-category-name';
    ...
    >
        ...
        >
            <h3 class="text-lg font-semibold text-slate-900 dark:text-slate-100">
                {{ isEditMode ? 'Edit Kategori Sparepart' : 'Tambah Kategori Sparepart' }}
            </h3>
            <button
                type="button"
        ...
        <div class="modal-scroll-green min-h-0 overflow-y-auto px-5 pb-5 pt-4">
            <form class="space-y-4" @submit.prevent="emit('submit')" @keydown.esc="handleEscKey" @keydown.enter="handleEnterKey">
                <InputField
                    id="owner-sparepart-category-name"
                    v-model="form.name"
                ...
                <div class="space-y-2">
                    <label class="text-sm font-semibold text-slate-700 dark:text-slate-200" for="owner-sparepart-category-description">
                        Deskripsi (Opsional)
                    </label>
                    <textarea
                        id="owner-sparepart-category-description"
                ...
                <p
                    v-if="errors?.create_sparepart_category && !form.errors.create_sparepart_category"
                    class="text-sm font-medium text-rose-600 dark:text-rose-300"
                >
                    {{ errors.create_sparepart_category }}
                </p>
                <p
                    v-if="errors?.update_sparepart_category && !form.errors.update_sparepart_category"
                    class="text-sm font-medium text-rose-600 dark:text-rose-300"
                >
                    {{ errors.update_sparepart_category }}
                </p>
