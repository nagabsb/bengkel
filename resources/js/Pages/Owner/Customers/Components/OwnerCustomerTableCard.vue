...
const columns = computed(() => [
    { key: 'name', label: 'Customer', sortable: true, headerClass: 'w-72' },
    { key: 'phone', label: 'No. HP', sortable: true, headerClass: 'w-40' },
    { key: 'email', label: 'Email', sortable: true, headerClass: 'w-56' },
...
const rows = computed(() => Array.isArray(props.customers?.data) ? props.customers.data : []);

const pagination = computed(() => ({
    mode: String(props.customers?.mode || 'cursor'),
    current_page: Number(props.customers?.current_page) || 1,
    last_page: Number(props.customers?.last_page) || 1,
    per_page: Number(props.customers?.per_page) || Number(props.filters?.per_page) || 10,
    total: Number(props.customers?.total) || 0,
    from: Number(props.customers?.from) || 0,
    to: Number(props.customers?.to) || 0,
    current_cursor: props.customers?.current_cursor ? String(props.customers.current_cursor) : null,
    next_cursor: props.customers?.next_cursor ? String(props.customers.next_cursor) : null,
    prev_cursor: props.customers?.prev_cursor ? String(props.customers.prev_cursor) : null,
    has_more_pages: Boolean(props.customers?.has_more_pages),
}));
</script>
...
<template>
    <article class="rounded-2xl border border-emerald-100 bg-white shadow-sm dark:border-emerald-500/20 dark:bg-slate-900">
        <header class="flex flex-wrap items-start justify-between gap-3 border-b border-slate-200 px-5 py-4 dark:border-slate-700">
            <div>
                <h3 class="text-lg font-semibold text-slate-900 dark:text-slate-100">Daftar Customer</h3>
                <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Kelola pelanggan untuk kebutuhan service dan transaksi.</p>
            </div>
            ...
            <div class="flex w-full flex-wrap items-center justify-end gap-2 text-xs font-semibold">
                ...
                <span class="rounded-full border border-slate-200 bg-slate-100 px-2.5 py-1 text-slate-600 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300">
                    Nonaktif: {{ customerSummary.inactive }}
                </span>
            </div>
        </header>
                ...
                <template #cell-actions="{ row }">
                    <div v-if="canManage" class="flex items-center justify-end gap-2">
                        ...
                        <button
                            ...
                            :disabled="formProcessing || deletingCustomerId === String(row.id)"
                            @click="emit('delete', row)"
                        >
                            {{ deletingCustomerId === String(row.id) ? 'Menghapus...' : 'Hapus' }}
                        </button>
                    </div>
