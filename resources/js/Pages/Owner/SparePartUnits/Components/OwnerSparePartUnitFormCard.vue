...
const firstInputId = 'owner-sparepart-unit-name';
    ...
    >
        ...
        >
            <h3 class="text-lg font-semibold text-slate-900 dark:text-slate-100">
                {{ isEditMode ? 'Edit Satuan Sparepart' : 'Tambah Satuan Sparepart' }}
            </h3>
            <button
                type="button"
        ...
        <div class="modal-scroll-green min-h-0 overflow-y-auto px-5 pb-5 pt-4">
            <form class="space-y-4" @submit.prevent="emit('submit')" @keydown.esc="handleEscKey" @keydown.enter="handleEnterKey">
                <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
                    <InputField
                        id="owner-sparepart-unit-name"
                        v-model="form.name"
                        label="Nama Satuan (Wajib)"
                        placeholder="Contoh: Pieces"
                        :error="form.errors.name"
                    />
                    <InputField
                        id="owner-sparepart-unit-symbol"
                        v-model="form.symbol"
                ...
                <div class="space-y-2">
                    <label class="text-sm font-semibold text-slate-700 dark:text-slate-200" for="owner-sparepart-unit-description">
                        Deskripsi (Opsional)
                    </label>
                    <textarea
                        id="owner-sparepart-unit-description"
                        ...
                        placeholder="Catatan tambahan satuan"
                    />
                ...
                <p
                    v-if="errors?.create_sparepart_unit && !form.errors.create_sparepart_unit"
                    class="text-sm font-medium text-rose-600 dark:text-rose-300"
                >
                    {{ errors.create_sparepart_unit }}
                </p>
                <p
                    v-if="errors?.update_sparepart_unit && !form.errors.update_sparepart_unit"
                    class="text-sm font-medium text-rose-600 dark:text-rose-300"
                >
                    {{ errors.update_sparepart_unit }}
                </p>
                ...
                >
                    {{ form.processing ? 'Menyimpan...' : (isEditMode ? 'Simpan Perubahan' : 'Tambah Satuan') }}
                </button>
            </form>
        </div>
    </article>
