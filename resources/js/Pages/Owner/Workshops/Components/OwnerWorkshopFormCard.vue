...
const firstInputId = 'owner-workshop-name';
    ...
    >
        ...
        >
            <h3 class="text-lg font-semibold text-slate-900 dark:text-slate-100">
                {{ isEditMode ? 'Edit Bengkel' : 'Tambah Bengkel' }}
            </h3>
            <button
                type="button"
        ...
        <div class="modal-scroll-green min-h-0 overflow-y-auto px-5 pb-5 pt-4">
            <form class="space-y-4" @submit.prevent="emit('submit')" @keydown.esc="handleEscKey" @keydown.enter="handleEnterKey">
                <InputField
                    id="owner-workshop-name"
                    v-model="form.name"
                    label="Nama Bengkel"
                    placeholder="Contoh: AutoServ Cabang Timur"
                    :required="true"
                    :error="form.errors.name"
                />

                <InputField
                    id="owner-workshop-code"
                    v-model="form.code"
                    label="Kode Bengkel (Otomatis)"
                    placeholder="Kode akan dibuat otomatis"
                    :readonly="true"
                    :error="form.errors.code"
                />
                <p class="-mt-2 text-xs text-slate-500 dark:text-slate-400">
                    Kode dibuat otomatis dari nama bengkel dan akan disesuaikan agar unik saat disimpan.
                </p>

                <InputField
                    id="owner-workshop-phone"
                    v-model="form.phone"
                    type="tel"
                    label="No. HP Bengkel"
                ...
                <div class="space-y-2">
                    <label class="text-sm font-semibold text-slate-700 dark:text-slate-200" for="owner-workshop-address">
                        Alamat Bengkel (Opsional)
                    </label>
                    <textarea
                        id="owner-workshop-address"
                    ...
                    />
                    <p v-if="form.errors.address" class="text-sm text-rose-600 dark:text-rose-300">{{ form.errors.address }}</p>
                </div>
                ...
                <p
                    v-if="errors?.create_workshop && !form.errors.create_workshop"
                    class="text-sm font-medium text-rose-600 dark:text-rose-300"
                >
                    {{ errors.create_workshop }}
                </p>
                <p
                    v-if="errors?.update_workshop && !form.errors.update_workshop"
                    class="text-sm font-medium text-rose-600 dark:text-rose-300"
                >
                    {{ errors.update_workshop }}
                </p>
                ...
                >
                    {{ form.processing ? 'Menyimpan...' : (isEditMode ? 'Simpan Perubahan' : 'Tambah Bengkel') }}
                </button>
            </form>
        </div>
    </article>
