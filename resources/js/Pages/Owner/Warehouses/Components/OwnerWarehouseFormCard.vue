...
const firstInputId = 'owner-warehouse-name';
    ...
    >
        ...
        >
            <h3 class="text-lg font-semibold text-slate-900 dark:text-slate-100">
                {{ isEditMode ? 'Edit Gudang' : 'Tambah Gudang' }}
            </h3>
            <button
                type="button"
        ...
        <div class="modal-scroll-green min-h-0 overflow-y-auto px-5 pb-5 pt-4">
            <form class="space-y-4" @submit.prevent="emit('submit')" @keydown.esc="handleEscKey" @keydown.enter="handleEnterKey">
                <div v-if="isWorkshopSelectable" class="grid min-w-0 gap-2" data-enter-ignore="true">
                    <label class="text-sm font-semibold text-slate-700 dark:text-slate-200" for="owner-warehouse-workshop">
                        Bengkel Tujuan
                    </label>
                    <AsyncSelect
                        id="owner-warehouse-workshop"
                ...
                <InputField
                    id="owner-warehouse-name"
                    v-model="form.name"
                    label="Nama Gudang (Wajib)"
                    placeholder="Contoh: Gudang Utama"
                    :error="form.errors.name"
                />

                <InputField
                    id="owner-warehouse-code"
                    v-model="form.code"
                    label="Kode Gudang (Opsional)"
                    placeholder="Contoh: GUD-UTAMA"
                    :error="form.errors.code"
                />

                <InputField
                    id="owner-warehouse-address"
                ...
                <div class="space-y-2">
                    <label class="text-sm font-semibold text-slate-700 dark:text-slate-200" for="owner-warehouse-notes">
                        Catatan (Opsional)
                    </label>
                    <textarea
                        id="owner-warehouse-notes"
                        v-model="form.notes"
                        rows="3"
                        ...
                        placeholder="Catatan tambahan gudang"
                    />
                ...
                <p
                    v-if="errors?.create_warehouse && !form.errors.create_warehouse"
                    class="text-sm font-medium text-rose-600 dark:text-rose-300"
                >
                    {{ errors.create_warehouse }}
                </p>
                <p
                    v-if="errors?.update_warehouse && !form.errors.update_warehouse"
                    class="text-sm font-medium text-rose-600 dark:text-rose-300"
                >
                ...
                >
                    {{ form.processing ? 'Menyimpan...' : (isEditMode ? 'Simpan Perubahan' : 'Tambah Gudang') }}
                </button>
            </form>
        </div>
    </article>
